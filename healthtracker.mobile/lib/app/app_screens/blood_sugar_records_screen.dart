...
class BloodSugarRecordsScreen extends StatefulWidget {
  const BloodSugarRecordsScreen({super.key});

  @override
  State<BloodSugarRecordsScreen> createState() => _BloodSugarRecordsScreenState();
}
...
class _BloodSugarRecordsScreenState extends State<BloodSugarRecordsScreen> {
  ...
  String _selectedStateFilter = 'All type';
  ...
  Future<void> _loadAllRecords() async {
    ...
    final list = await DatabaseHelper.instance.getAllSugarRecords();
    setState(() {
      _records = list;
      _isLoading = false;
    });
  ...
  List<BloodSugarRecord> get _filteredRecords {
    List<BloodSugarRecord> result = List.from(_records);

    if (_selectedStateFilter != 'All type') {
      result = result.where((rec) => rec.state == _selectedStateFilter).toList();
    }
