...
class HeartRateRecordsScreen extends StatefulWidget {
  const HeartRateRecordsScreen({super.key});

  @override
  State<HeartRateRecordsScreen> createState() => _HeartRateRecordsScreenState();
}

class _HeartRateRecordsScreenState extends State<HeartRateRecordsScreen> {
  ...
  final List<String> _stateFilters = ['All', 'RESTING', 'EXERCISE'];
  ...
  Future<void> _loadAllRecords() async {
    ...
    final list = await DatabaseHelper.instance.getAllHeartRateRecords();
    setState(() {
      _records = list;
      _isLoading = false;
    });
  ...
  List<HeartRateRecord> get _filteredRecords {
    List<HeartRateRecord> result = List.from(_records);

    if (_selectedStateFilter != 'All') {
      result = result.where((rec) => rec.status == _selectedStateFilter).toList();
    }
